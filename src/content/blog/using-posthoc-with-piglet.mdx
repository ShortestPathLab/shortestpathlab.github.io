...
```bash
python piglet.py -p ./example/arena2.map.scen -f graph -s breadth --log trace-file -t 0.05 -x 2 -n 1
```

<WorkspaceButton id="bfs" name="BFS on Arena 2" />

```bash
python piglet.py -p ./example/arena2.map.scen -f graph -s depth --log trace-file -t 0.05 -x 2 -n 1
```
